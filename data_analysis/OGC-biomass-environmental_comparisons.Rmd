...
ANOVAs were conducted for discharge, precipitation, water temperature, and air temperature to test the effects of period, season, and the interaction with Type II sums-of-squares. ANOVA assumptions were inspected graphically using `check_model()`. Post-hoc Tukey's HSD tests were conducted using `HSD.test()` to examine for differences among groups within the ANOVA. Effect sizes for the ANOVAs were calculated as $\eta^2_P$ using `eta_squared()`.
